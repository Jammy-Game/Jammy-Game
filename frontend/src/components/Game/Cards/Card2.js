  ...
  useEffect(() => {
    ...
    if (gameStatus === 3) {
      dispatch(
        ...
        .then((result) => {
          if (result.payload && !card2DrawnNumbers.length > 0) {
            console.log(
              `closeNumber-${gameId}-${playerCardIndex}`,
              result.payload
            );
            setCard2DrawnNumbers(result.payload);
          }
        })
        .catch((error) => console.log(error));
    }

    if (unSigner.contract && cardCount >= 3) {
      ...
      dispatch(
        ...
        .then((result) => {
          setIsLoading(true);
          //console.log("getCard(2):", result.payload);
          if (result.payload) {
              ...
              ? cardNumbers.map((number, numberindex) =>
                  manuelCloseToggle ? (
                    ...
                    <div
                      ...
                      className={
                        card2DrawnNumbers
                          ? card2DrawnNumbers.includes(number) || number === 0
                            ? "item revealed"
                            : "item"
                          : "item"
                      }
