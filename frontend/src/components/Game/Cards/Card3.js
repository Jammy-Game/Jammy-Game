  ...
  useEffect(() => {
    ...
    if (gameStatus === 3) {
      dispatch(
        ...
        .then((result) => {
          if (result.payload && !card3DrawnNumbers.length > 0) {
            console.log(
              `closeNumber-${gameId}-${playerCardIndex}`,
              result.payload
            );
            setCard3DrawnNumbers(result.payload);
          }
        })
        .catch((error) => console.log(error));
    }

    if (unSigner.contract && cardCount >= 4) {
      ...
      dispatch(
        ...
        .then((result) => {
          setIsLoading(true);
          //console.log("getCard(3):", result.payload);
          if (result.payload) {
              ...
              ? cardNumbers.map((number, numberindex) =>
                  manuelCloseToggle ? (
                    ...
                    <div
                      ...
                      className={
                        card3DrawnNumbers
                          ? card3DrawnNumbers.includes(number) || number === 0
                            ? "item revealed"
                            : "item"
                          : "item"
                      }
